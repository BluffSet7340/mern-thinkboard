...
const RedirectAuthenticatedUser = ({children}) => {
  ...
  if(isAuthenticated && user.isVerified){ //so anytime a logged in user tries to get into the signup page we 
...
const ProtectedRoute = ({children}) => {
  ...
  if(!isAuthenticated){ // if the user is not authenticated, then prompt to log em in
    return <Navigate to="/login" replace/>
  }

  if(!user.isVerified){ // if they have not verified their account, take them to the verify-email page
